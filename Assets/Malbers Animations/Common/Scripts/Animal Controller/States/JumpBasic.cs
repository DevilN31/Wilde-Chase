...
namespace MalbersAnimations.Controller
{
    public class JumpBasic : State
    {
        ...
        {
            if (InputValue && JumpsPerformanced < JumpAmount && CanJumpAgain && !InMainTagHash) // MEANS WE ARE NOT ON THE SAME ANIMATION
            {
                Activate();
                JumpsPerformanced++;
            }
        ...
        public override void NewActiveState(StateID newState)
        {
            ...
            if (newState == StateEnum.Fall && animal.LastState.ID != ID) //If we were not jumping then increase the Double Jump factor
            {
