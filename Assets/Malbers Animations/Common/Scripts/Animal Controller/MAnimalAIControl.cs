      ...
      //  public bool NullDestination => DestinationPosition == NullVector;
        ...
        private void CheckWalkDistance()
        {
            if (IsGrounded && walkDistance > 0)
            {
                ...
                {
                    if (animal.CurrentSpeedIndex != animal.CurrentSpeedSet.StartVerticalIndex)
                        animal.CurrentSpeedIndex = animal.CurrentSpeedSet.StartVerticalIndex;  //Restore the Current Speed Index to the Speed Set
                }
            }
        }
