...
namespace MalbersAnimations.Utilities
{
    ...
    public class LookAt : MonoBehaviour, IAnimatorListener 
    {
        ...
        void LateUpdate()
        {
            ...
            if (OnlyTargets) Active = (aimer.AimTarget != null);        //If Only Target is true then Disable it because we do not have any target
            if (!OnlyTargets) Active = (aimer.MainCamera != null);      //If Only Target is false and there's no Camera then Disable it because we do not have any target
