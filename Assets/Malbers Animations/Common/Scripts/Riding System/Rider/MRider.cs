...
namespace MalbersAnimations.HAP
{
    ...
    public class MRider : MonoBehaviour, IAnimatorListener
    {
        ...
        /// <summary>  Store All colliders that are enabled and not Trigger </summary>
        ...
        void Start()
        {
            ...
            if ((int)LinkUpdate == 0 || !Parent)
                LinkUpdate = UpdateMode.FixedUpdate | UpdateMode.LateUpdate;
                                  ...
                                  //Set All if Started Mounted is Active   

            if (RiderRoot == null) RiderRoot = transform.root;
            if (Anim == null) Anim = GetComponentInParent<Animator>() ?? GetComponentInChildren<Animator>();
            if (RB == null) RB = GetComponentInParent<Rigidbody>() ?? GetComponentInChildren<Rigidbody>();
        ...
        internal virtual void End_Dismounting()
        {
            ...
            RiderRoot.rotation = Quaternion.FromToRotation(RiderRoot.up, -Physics.gravity) * RiderRoot.rotation;    //Reset the Up Vector; ****IMPORTANT when  CHANGE THE GRAVIY
        ...
        private void SolveStraightMount()
        {
            if (IsRiding)
            {
                if (Montura.StraightSpine && !CombatAim)
                {
        ...
        private void Reset()
        {
            animator = GetComponent<Animator>() ?? GetComponentInParent<Animator>() ?? GetComponentInChildren<Animator>();
